...
const generateConvergingParticles = (count: number) => {
  return Array.from({ length: count }).map((_, i) => ({
    id: i,
...
const HeroSection = () => {
  ...
  return (
    <section
      id="hero"
      ref={containerRef}
      className="min-h-[100dvh] flex flex-col items-center justify-center px-6 pt-16 relative overflow-hidden bg-background"
    >
      ...
      <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden">
        ...
        {ambientParticles.map((p) => (
          <motion.div
            ...
            animate={{
              x: [`${p.x}vw`, `${p.x + p.dx}vw`, `${p.x}vw`],
              y: [`${p.y}vh`, `${p.y + p.dy}vh`, `${p.y}vh`],
              opacity: [0, 0.8, 0]
            }}
      ...
      <div className="text-center relative z-10 flex flex-col items-center w-full max-w-4xl mx-auto">
        <motion.p
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 1.5, duration: 0.8 }}
          className="font-body text-sm md:text-base uppercase tracking-[0.3em] sm:tracking-[0.4em] text-foreground/90 mb-6"
        >
          August 2026
        </motion.p>
        ...
        <div className="relative group cursor-crosshair">
          <h1 className="font-display font-extrabold uppercase text-3xl sm:text-5xl md:text-6xl lg:text-7xl leading-none flex justify-center flex-nowrap gap-x-0.5 sm:gap-x-1 md:gap-x-2 relative z-10">
        ...
        >
          ...
          <a
            href="#about"
            className="font-body text-xs uppercase tracking-widest text-primary border-b border-primary/40 pb-1 hover:border-primary transition-colors hover:text-white duration-300 w-full sm:w-auto text-center sm:text-left sm:mt-0"
          >
            Learn More
          </a>
        </motion.div>
